...
def get_pnl():
    ...
    query = '''
    SELECT holdings.Ticker, Company_Name, Purchase_Price, Shares 
    FROM holdings INNER JOIN stocks 
    ON holdings.Ticker = stocks.Ticker;
    '''
    ...
    holdingsdict = {}
    for h in holdings:
        if h[0] not in holdingsdict:
            holdingsdict[h[0]] = [h[1], h[3], float(h[2]) * h[3]]
        else:
            shares = holdingsdict[h[0]][1] + h[3]
            costs = holdingsdict[h[0]][2] + float(h[2]) * h[3]
            holdingsdict[h[0]] = [h[1], shares, costs]
    
    for k in holdingsdict:
        gain_dict[holdingsdict[k][0]] = get_price(k) * holdingsdict[k][1] - holdingsdict[k][2]
...
def get_price(ticker):
    ...
    try:
        priceStr = response_page.find('span', class_="pr").get_text()
